...
import { enableUserLocation, enableNotification } from '../services/auth/auth.api';
  ...
  const isLocationServicesEnabled = async () => {
    if (Platform.OS === 'android') {
      return new Promise((resolve) => {
        Geolocation.getCurrentPosition(
          () => resolve(true),
          (error) => {
            logger.log("android geolocation error", error);
            resolve(error.code !== 2);
          },
          { timeout: 1000, maximumAge: 0, enableHighAccuracy: false }
        );
    ...
    return new Promise((resolve) => {
      Geolocation.getCurrentPosition(
        () => resolve(true),
        (error) => {
          logger.log('iOS location error code:', error.code);
          if (error.code === 2) {
            // Only code 2 = GPS hardware switch is OFF
            resolve(false);
          } else {
            // code 1 = permission, code 3 = TIMEOUT (not GPS off!)
            // ✅ Timeout does NOT mean services disabled
            resolve(true);
          }
        },
        {
          timeout: 8000,  // ✅ Give enough time for "Ask Every Time" prompt
          maximumAge: 0,
          enableHighAccuracy: false
        }
      );
...
logger.log("-======currentStatus======",currentStatus);
    ...
    // 3. If Granted, check if the GPS toggle is actually ON
  ...
  const syncPermissionsWithBackend = async () => {
    ...
    await enableNotification({ notification_enabled: notifGranted })
    await enableUserLocation({ location_enabled: locGranted })
  }
