...
const LoadingScreen = ({ 
  ...
  useEffect(() => {
    ...
    // Scooter Vibration loop
    const driveSubscription = Animated.loop(
      Animated.sequence([
        Animated.timing(driveAnim, {
          toValue: 1,
          duration: 100,
    ...
    // Progress bar animation
    const progressSubscription = Animated.loop(
      Animated.timing(progressAnim, {
        toValue: 1,
        duration: 1500,
        easing: Easing.bezier(0.25, 0.1, 0.25, 1),
    ...
    // Text updates with proper cleanup
    const textTimer1 = setTimeout(() => setLoadingText('Checking permissions'), 800);
    const textTimer2 = setTimeout(() => setLoadingText('Getting everything ready'), 1600);
    const textTimer3 = setTimeout(() => setLoadingText('Almost there'), 2400);
...
// ✅ FIXED: LoadingDot with proper cleanup
const LoadingDot = ({ delay }: { delay: number }) => {
  const bounceAnim = useRef(new Animated.Value(0)).current;
  const animationRef = useRef<any>(null);

  useEffect(() => {
    // ✅ Delay animation start to prevent early memory leak
    const timeoutId = setTimeout(() => {
      animationRef.current = Animated.loop(
        Animated.sequence([
          Animated.timing(bounceAnim, { 
