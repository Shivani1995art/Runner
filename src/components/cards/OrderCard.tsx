...
// import { StyleSheet, Text, View, TouchableOpacity } from 'react-native';
// import { MapPin, Clock, Map, CheckCircle } from 'lucide-react-native';
// import { vs, ms, fontSize, hp, wp } from '../../utils/responsive';
// import Colors from '../../utils/colors';
// import { Typography } from '../../utils/typography';
// import { useFocusEffect } from '@react-navigation/native';
// import { logger } from '../../utils/logger';
...
//         <>
//           <CheckCircle color={clockColor} size={ms(16)} strokeWidth={2.5} />
//           <Text style={[styles.timeText, timeText, { color: clockColor,width: wp(20) }]} numberOfLines={1}>
//             Order Ready
//           </Text>
//         </>
//       );
...
//       <>
//         <Clock color={clockColor} size={ms(16)} strokeWidth={2.5} />
//         <Text style={[styles.timeText, timeText, { color: clockColor ,width: wp(14)}]}  numberOfLines={1}>
//           {formatTime(remainingSeconds)}
//         </Text>
//       </>
//     );
...
const OrderCard: React.FC<OrderCardProps> = ({
  ...
  // ✅ Use ref to store interval ID so it doesn't interfere with other cards
  const timerIntervalRef = useRef<NodeJS.Timeout | null>(null);

  // ✅ Calculate initial seconds - memoized so it doesn't change unnecessarily
  const getInitialSeconds = useMemo(() => {
    return (): number => {
      // Priority 1: use estimatedReadyAt ISO timestamp via moment (most accurate)
      if (estimatedReadyAt) {
        const diff = moment(estimatedReadyAt).diff(moment(), 'seconds');
  ...
  }, [estimatedReadyAt, time, startedAt]);

  const [remainingSeconds, setRemainingSeconds] = useState<number>(getInitialSeconds());

  // ✅ FIXED: Separate timer effect with proper cleanup
  useEffect(() => {
    // ✅ Only run timer for 'preparing' status
    if (status !== 'preparing') {
      logger.log(`⏭️ [OrderCard ${orderID}] Not preparing, skipping timer`);
      return;
    }

    logger.log(`⏱️ [OrderCard ${orderID}] Starting timer`);
    ...
    timerIntervalRef.current = setInterval(() => {
      setRemainingSeconds((prev) => {
        ...
        if (newValue % 10 === 0) {
          logger.log(`⏱️ [OrderCard ${orderID}] Time remaining: ${newValue}s`);
        }
    ...
    return () => {
      if (timerIntervalRef.current) {
        logger.log(`🛑 [OrderCard ${orderID}] Stopping timer`);
        clearInterval(timerIntervalRef.current);
        timerIntervalRef.current = null;
      }
    };
  }, [status, estimatedReadyAt, startedAt, orderID, getInitialSeconds]);
